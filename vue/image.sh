...
echo ">>> Building Dist"
docker run -i --rm --privileged -v $(pwd):/src -w /src node:latest bash -c "npm install --registry=https://registry.npm.taobao.org && npm run build $deploy"
